...
package body Application.Telemetry is
   ...
   procedure Send_Location is
      ...
      Payload_Length : Application.Network.Payload_Index_Type := 1023;
   ...
   begin
      ...
      New_Packet :=
        Application.Network.Packet_Type'
          (Packet_Variant => Application.Network.Telemetry, Packet_Number => 45,
           Source => Application.Network.Device_Identifier, Target => 0,
           Payload_Length => Payload_Length, Payload => Payload,
           Broadcast      => Application.Network.True);

      Telemetry_Packet.Location.Position.Longitude :=
        Float_Type(Application.State.Current_State.Physical_State.Position.Longitude);
      Telemetry_Packet.Location.Position.Latitude :=
        Float_Type(Application.State.Current_State.Physical_State.Position.Latitude);
      Telemetry_Packet.Location.Position.Altitude :=
         Float_Type(Application.State.Current_State.Physical_State.Position.Altitude);
